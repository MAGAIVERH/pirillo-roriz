...
export type StudentWarningCatalogItem = {
  id: string;
  title: string;
  content: string;
  type: WarningType;
...
/** Avisos visíveis para um aluno (admin + professor via turma). */
export async function getStudentWarningsCatalog(
  studentId: string,
): Promise<StudentWarningCatalogItem[]> {
