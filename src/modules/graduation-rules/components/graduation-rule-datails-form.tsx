...
import { updateGraduationRuleAction } from '@/modules/graduation-rules/actions/update-graduation-rule';
...
type GraduationRuleDetailsFormProps = {
  rule: {
    id: string;
    program: 'KIDS' | 'ADULT';
...
export const GraduationRuleDetailsForm = ({
  rule,
  belts,
}: GraduationRuleDetailsFormProps) => {
  const [program, setProgram] = useState<'KIDS' | 'ADULT'>(rule.program);
  const [currentBeltId, setCurrentBeltId] = useState(rule.currentBeltId);
  const [currentDegreeId, setCurrentDegreeId] = useState(
    rule.currentDegreeId || '__none__',
  );
  const [nextBeltId, setNextBeltId] = useState(rule.nextBeltId);
  const [nextDegreeId, setNextDegreeId] = useState(
    rule.nextDegreeId || '__none__',
  );
  const [minimumMonths, setMinimumMonths] = useState(rule.minimumMonths);
  const [minimumAge, setMinimumAge] = useState(rule.minimumAge);
  const [status, setStatus] = useState<'ACTIVE' | 'INACTIVE'>(rule.status);
  ...
  const handleSubmit = () => {
    ...
    startTransition(async () => {
      const result = await updateGraduationRuleAction({
        ruleId: rule.id,
        ...parsed.data,
      });
